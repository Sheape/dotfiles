...
;;   (add-hook 'python-ts-mode #'python-flycheck-setup)
;;   )
(use-package lsp-ui)
(lsp-ui-mode 1)

(custom-set-faces
 ;; Change the color for LSP errors
 `(flycheck-error ((t (:underline (:color ,(catppuccin-get-color 'red) :style wave)))))
 `(flymake-error ((t (:underline (:color ,(catppuccin-get-color 'red) :style wave)))))

 ;; Change the color for LSP warnings
 `(flycheck-warning ((t (:underline (:color ,(catppuccin-get-color 'peach) :style wave)))))
 `(flymake-warning ((t (:underline (:color ,(catppuccin-get-color 'peach) :style wave)))))

 ;; Change the color for LSP notes/information
 `(flycheck-info ((t (:underline (:color ,(catppuccin-get-color 'green) :style wave)))))
 `(flymake-note ((t (:underline (:color ,(catppuccin-get-color 'green) :style wave)))))

 ;; Change the color for highlighted symbols
 `(eglot-highlight-symbol-face ((t (:weight bold :foreground ,(catppuccin-get-color 'teal))))))
...
(use-package jtsx
  ...
  (add-hook 'jtsx-jsx-mode-hook 'jtsx-bind-keys-to-jsx-mode-map)
  (add-hook 'jtsx-tsx-mode-hook 'jtsx-bind-keys-to-tsx-mode-map))
...
;; Uncomment this if you want codeium
;; (use-package company
;;   :defer 0.1
;;   :config
...
;; Uncomment if you want Ruff as the LSP server of choice
...
(defun my/python-save-actions ()
  "Sort imports and fix missing imports on save."
  (when (derived-mode-p 'python-ts-mode)
    (ruff-format-buffer)
    (python-fix-imports)
    ))

(add-hook 'before-save-hook #'my/python-save-actions)
...
(add-to-list 'exec-path (expand-file-name (concat (getenv "HOME") "/.local/share/pnpm")))

(require 'dap-python)
(setq dap-python-debugger 'debugpy)

(dap-register-debug-template "debugpy"
                             (list :type "python"
                                   :args "-i"
                                   :cwd nil
                                   :env '(("DEBUG" . "1"))
...
(use-package! yasnippet
  ...
  ;; Unbind the default TAB key
  (define-key yas-minor-mode-map (kbd "TAB") nil)
  (define-key yas-minor-mode-map (kbd "<tab>") nil)

  ;; Bind a new key for expanding snippets
  (define-key yas-minor-mode-map (kbd "<f1>") 'yas-expand))

;; Custom key binding for python-mode
(add-hook 'python-ts-mode-hook
          (lambda ()
            (local-set-key (kbd "<f1>") 'yas-expand)
            ))

(defun my/python-ts-mode-setup ()
  "Set up Python environment when entering python-ts-mode."
  (pipenv-mode)             ;; Activate pipenv-mode
  (pipenv-activate)         ;; Activate the pipenv environment
  )
